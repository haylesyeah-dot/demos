...
Streamlit in Snowflake · v3 (matches React demo exactly)
"""
...
[data-testid="metric-container"] {{
    background: {CARD}; border: 1.5px solid {BORDER}; border-radius: 12px; padding: 16px 18px;
}}
...
[data-testid="stTabs"] button {{
    ...
    border-radius: 0 !important; padding: 8px 22px !important;
...
.stButton > button[kind="primary"] {{
    ...
    font-weight: 700 !important; font-size: 13px !important; padding: 10px 28px !important;
}}
...
.stButton > button[kind="secondary"]:hover {{ color: {WHITE} !important; border-color: {CYAN} !important; }}

[data-testid="stMultiSelect"] > div > div {{ background: {CARD} !important; border-color: {BORDER} !important; }}
...
[data-testid="stDataFrame"] td {{ background: {CARD} !important; color: #e8e0ff !important; font-size: 12px !important; }}

.stAlert {{ border-radius: 10px !important; }}
hr {{ border-color: {BORDER} !important; margin: 8px 0 !important; }}
code {{ color: {CYAN} !important; background: {CYAN}15 !important; border-radius: 4px; padding: 2px 6px; }}
pre {{ background: {BG_DEEP} !important; border: 1px solid {BORDER} !important; border-radius: 10px !important; padding: 20px !important; }}
[data-testid="stProgress"] > div > div {{ background: linear-gradient(90deg, {PURPLE}, {PURPLE_LT}) !important; }}
</style>
...
SF_OBJECTS = [
    ("BRAND_IDS_EXT",   "EXTERNAL TABLE",  "Queryable layer over your staged hashed files"),
      ...
      <div style="display:flex;align-items:center;gap:10px;">
        ...
        <div>
          <div style="font-size:14px;font-weight:900;color:white;letter-spacing:1.2px;line-height:1.1;">
            NUMAD<span style="color:{CYAN};">.AI</span>
          </div>
...
def nav_row():
    nav_pages = [
        ...
        ("activate",  "03 · Activate Audiences"),
        ("measure",   "04 · Measure Campaigns"),
        ("techspec",  "Spec"),
    ]
    cols = st.columns(len(nav_pages))
    for col, (pid, label) in zip(cols, nav_pages):
        with col:
            t = "primary" if st.session_state.page == pid else "secondary"
            if st.button(label, key=f"nav_{pid}", type=t, use_container_width=True):
...
def back_btn(label, target):
    if st.button(f"← {label}", key=f"back_{target}", type="secondary"):
...
def callout(text, color=GREEN, icon="🛡️"):
    st.markdown(f"""
    <div style="background:{color}0D;border:1px solid {color}30;border-radius:9px;
...
def stat_tile_row(tiles):
    """tiles = list of (value, label, note, color)"""
    cols = st.columns(len(tiles))
    for col, (v, l, n, c) in zip(cols, tiles):
        with col:
            st.markdown(f"""
            <div style="background:{SURFACE};border-radius:9px;padding:12px 10px;text-align:center;">
...
def chart_roas_bars(height=220):
    fig = go.Figure(go.Bar(
        x=[r[1] for r in ROAS_CHANNELS],
        y=[r[0] for r in ROAS_CHANNELS],
        orientation="h",
        marker_color=[r[2] for r in ROAS_CHANNELS],
        text=[f"{r[1]}x" for r in ROAS_CHANNELS],
...
def chart_venn():
    ...
    fig.add_shape(type="circle", x0=0.04, y0=0.08, x1=0.64, y1=0.92,
                  fillcolor="rgba(124,58,237,0.22)", line_color=PURPLE_LT,
...
def chart_signal_coverage():
    ...
    fig.update_layout(
        ...
        legend=dict(font=dict(size=10, color=MUTED), orientation="h", y=-0.18),
        xaxis=dict(range=[0, 105], ticksuffix="%",
                   tickfont=dict(color=MUTED, size=10), gridcolor=BORDER),
        yaxis=dict(tickfont=dict(color=MUTED, size=11)),
    )
    return fig
...
def page_overview():
    ...
    lc, rc = st.columns(2)
    with lc:
        st.markdown(f"<div style='color:{MUTED};font-size:9px;letter-spacing:2px;font-weight:700;margin-bottom:6px;'>ROAS BY CHANNEL</div>", unsafe_allow_html=True)
        st.plotly_chart(chart_roas_bars(), use_container_width=True, config={"displayModeBar": False})
    with rc:
        st.markdown(f"<div style='color:{MUTED};font-size:9px;letter-spacing:2px;font-weight:700;margin-bottom:6px;'>6-WEEK ROAS TREND</div>", unsafe_allow_html=True)
    ...
    steps = [
        ...
        ("measure",  "04 · Measure Campaigns",   GREEN,     "Closed-loop attribution inside Snowflake. No raw data shared."),
    ]
    cols = st.columns(4)
...
def page_ingest():
    ...
    with left:
        st.markdown(f"""
        <div style="background:{CARD};border:1.5px solid {BORDER};border-radius:14px;padding:14px 16px;margin-bottom:10px;">
          <div style="display:flex;align-items:center;gap:9px;margin-bottom:4px;">
            <div style="width:30px;height:30px;background:#29B5E818;border:1px solid #29B5E835;border-radius:8px;
                 display:flex;align-items:center;justify-content:center;flex-shrink:0;">{SF_SVG}</div>
        ...
        st.markdown(f"<div style='color:{MUTED};font-size:9px;font-weight:700;letter-spacing:1.5px;margin-bottom:8px;'>SELECT YOUR DATA TYPES</div>", unsafe_allow_html=True)
        native_opts = [f"{tag} — {label}" for tag, label, _ in NATIVE_SOURCES]
        ...
        if sel_native:
            ...
            st.dataframe(
                pd.DataFrame([{"Tag": s[0], "Source": s[1], "Identity Fields": s[2]} for s in matched]),
                hide_index=True, use_container_width=True,
            )

        st.markdown(f"<div style='color:{MUTED};font-size:9px;font-weight:700;letter-spacing:1.5px;margin:12px 0 8px;'>SNOWFLAKE OBJECTS NUMAD PROVISIONS IN YOUR ACCOUNT</div>", unsafe_allow_html=True)
        st.dataframe(
            pd.DataFrame([{"Object": n, "Type": t, "Description": d} for n, t, d in SF_OBJECTS]),
            hide_index=True, use_container_width=True,
        )
    ...
    with right:
        st.markdown(f"""
        <div style="background:{CARD};border:1.5px solid {PURPLE_LT}40;border-radius:14px;padding:14px 16px;margin-bottom:10px;">
          <div style="display:flex;align-items:center;gap:9px;margin-bottom:4px;">
            <div style="width:30px;height:30px;background:{PURPLE_LT}18;border:1px solid {PURPLE_LT}35;
                 border-radius:8px;display:flex;align-items:center;justify-content:center;flex-shrink:0;font-size:14px;">🔗</div>
            <b style="color:{WHITE};font-size:13px;">Open Flow Connectors</b>
          </div>
          <div style="color:{MUTED};font-size:11px;">OAuth-authenticated sync — no manual file exports required</div>
        </div>
        """, unsafe_allow_html=True)

        st.markdown(f"<div style='color:{MUTED};font-size:9px;font-weight:700;letter-spacing:1.5px;margin-bottom:8px;'>SELECT CONNECTORS TO AUTHENTICATE</div>", unsafe_allow_html=True)
        of_opts = [n for n, *_ in OF_CONNECTORS]
        ...
        if sel_of:
            ...
            st.dataframe(
                pd.DataFrame([{"Connector": n, "Category": cat, "Identity Fields": ids}
                               for n, cat, ids in matched_of]),
                hide_index=True, use_container_width=True,
            )
            callout("✓ OAuth handshake completes at ingestion — data syncs directly into Snowflake", PURPLE_LT, "")
        callout("Saudi PDPL & SDAIA compliant — purpose limitation enforced at the Snowflake object level for both ingestion paths", GREEN, "🛡️")
    ...
    if not st.session_state.ingest_done:
        ...
        if sel_of:     parts.append(f"{len(sel_of)} Open Flow connector{'s' if len(sel_of)>1 else ''}")
        st.success(f"✅ Ingestion staged in Snowflake — {' · '.join(parts)}")
...
def page_graph():
    ...
    if not st.session_state.graph_done:
        c1, c2 = st.columns(2)
        with c1:
            st.markdown(f"<div style='color:{CYAN};font-size:9px;letter-spacing:2px;font-weight:700;margin-bottom:10px;'>READY TO GRAPH</div>", unsafe_allow_html=True)
            ...
            for val, lbl in [("82K", "Your ingested IDs"), (str(n_src), "Sources selected"), ("3", "Signal types (before)")]:
                ...
                """, unsafe_allow_html=True)
        with c2:
            st.markdown(f"<div style='color:{MUTED};font-size:9px;letter-spacing:2px;font-weight:700;margin-bottom:10px;'>GRAPH OBJECTS TO BE CREATED</div>", unsafe_allow_html=True)
            st.dataframe(
                pd.DataFrame([{"Object": n, "Type": t, "Description": d} for n, t, d in SF_OBJECTS[2:]]),
                hide_index=True, use_container_width=True,
            )
        st.markdown("<div style='height:8px'></div>", unsafe_allow_html=True)
        ...
        callout(
            "87% of your 82K IDs were matched in the Numad graph. Matched profiles are enriched "
            "with new signals, and <b>66K additional profiles</b> are reachable via household and "
            "device links. Total addressable: <b>137K</b> (+67% vs. input).",
            GREEN, "",
        )

        r1, r2 = st.columns(2)
        with r1:
            st.markdown(f"<div style='color:{MUTED};font-size:9px;letter-spacing:2px;font-weight:700;margin-bottom:4px;'>OVERLAP DIAGRAM</div>", unsafe_allow_html=True)
            ...
            <div style="background:{GREEN}08;border:1px solid {GREEN}22;border-radius:8px;
              ...
              <span style="font-size:18px;font-weight:900;color:{GREEN};">137K
                <span style="font-size:11px;font-weight:600;">(+67%)</span>
              </span>
            </div>
            """, unsafe_allow_html=True)

        with r2:
            st.markdown(f"<div style='color:{MUTED};font-size:9px;letter-spacing:2px;font-weight:700;margin-bottom:4px;'>SIGNAL COVERAGE (matched IDs)</div>", unsafe_allow_html=True)
            st.plotly_chart(chart_signal_coverage(), use_container_width=True, config={"displayModeBar": False})

        st.markdown(f"<div style='color:{MUTED};font-size:9px;letter-spacing:2px;font-weight:700;margin:12px 0 8px;'>SEGMENT EXPANSION</div>", unsafe_allow_html=True)
        st.dataframe(
            pd.DataFrame([{"Segment": n, "Before": b, "After (enriched)": a, "Lift": lift}
...
def page_activate():
    ...
    step_header(
        "STEP 3 OF 4", "Activate Audiences",
    ...
    if not st.session_state.act_launched:
        ...
        with tab_rmn:
            st.dataframe(
                pd.DataFrame([{"Partner": n, "Reach": r, "Formats": fmt} for n, r, fmt in RMN_PARTNERS]),
                hide_index=True, use_container_width=True,
            )
            rmn_opts = [n for n, *_ in RMN_PARTNERS]
        ...
        with tab_dsp:
            st.dataframe(
                pd.DataFrame([{"Partner": n, "Reach": r, "Signal": sig, "Type": t}
                               for n, r, sig, t in DSP_PARTNERS]),
                hide_index=True, use_container_width=True,
            )
            dsp_opts = [n for n, *_ in DSP_PARTNERS]
        ...
        if all_sel:
            ...
            if st.button(f"⚡  Launch Activation to {len(all_sel)} partner(s)", type="primary"):
                st.session_state.act_launched = True
                st.rerun()
        else:
            st.info("Select at least one partner above to activate.")
    else:
        st.success(
            f"✅ Activation launched — 137K hashed profiles sent to **{', '.join(st.session_state.act_partners)}** via Snowflake.\n\n"
            "Audiences will be available within 2–4 hours."
        )
        c1, c2 = st.columns(2)
        with c1:
            if st.button("Add more partners", type="secondary"):
                st.session_state.act_launched = False
                st.rerun()
        with c2:
...
def page_measure():
    ...
    lc, rc = st.columns(2)
    with lc:
        st.markdown(f"<div style='color:{MUTED};font-size:9px;letter-spacing:2px;font-weight:700;margin-bottom:6px;'>ROAS BY ACTIVATION CHANNEL</div>", unsafe_allow_html=True)
        st.plotly_chart(chart_roas_bars(), use_container_width=True, config={"displayModeBar": False})
        st.markdown(f"<div style='color:{MUTED};font-size:9px;letter-spacing:2px;font-weight:700;margin:10px 0 6px;'>6-WEEK ROAS TREND</div>", unsafe_allow_html=True)
        st.plotly_chart(chart_roas_trend(), use_container_width=True, config={"displayModeBar": False})

    with rc:
        st.markdown(f"<div style='color:{MUTED};font-size:9px;letter-spacing:2px;font-weight:700;margin-bottom:8px;'>INCREMENTALITY MEASUREMENT</div>", unsafe_allow_html=True)
        st.dataframe(
            pd.DataFrame([
...
def page_techspec():
    ...
    with tab_arch:
        c1, c2 = st.columns(2)
        with c1:
            st.markdown(f"<div style='color:{CYAN};font-size:9px;letter-spacing:2px;font-weight:700;margin-bottom:10px;'>SECURE DATA COLLABORATION CONFIG</div>", unsafe_allow_html=True)
            st.dataframe(
                pd.DataFrame([
        ...
        with c2:
            st.markdown(f"<div style='color:{PURPLE_LT};font-size:9px;letter-spacing:2px;font-weight:700;margin-bottom:10px;'>IDENTITY NAMESPACES</div>", unsafe_allow_html=True)
            st.dataframe(
                pd.DataFrame([
                ...
                ], columns=["Namespace", "Type", "Coverage"]),
                hide_index=True, use_container_width=True,
            )
